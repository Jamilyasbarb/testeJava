...
package com.mycompany.testejavajunior;
...
public class TesteJavaJunior {
   ...
   public static void main(String[] args) {
            ...
            if (usuariosSearch.size() > 0) {
                ...
                if ((usuarioLogado.getSenha().equals(senha))) {
                    if (usuarioLogado.IsEmpresa()) {
                        acessoEmpresa(usuarios, clientes, empresas, produtos, carrinho, vendas, usuarioLogado);
                    } else if(usuarioLogado.IsAdmin()){
                            acessoAdmin(usuarios, clientes, empresas, produtos, carrinho, vendas, usuarioLogado);
                    }else {
                        acessoCliente(usuarios, clientes, empresas, produtos, carrinho, vendas, usuarioLogado);
                    }
	...
	public static Venda criarVenda(List<Produto> carrinho, Empresa empresa, Cliente cliente, List<Venda> vendas) {
            ...
            double totalComTaxa = total - comissaoSistema;
            empresa.setSaldo(empresa.getSaldo() + totalComTaxa);
            vendas.add(venda);
            return venda;
	}
        
        public static void acessoAdmin(List<Usuario> usuarios, List<Cliente> clientes, List<Empresa> empresas,
			List<Produto> produtos, List<Produto> carrinho,List<Venda> vendas,Usuario usuarioLogado){
            System.out.println("\n**************************************************************************");
            System.out.printf("OLÁ %s, escolha uma opção para iniciar %n", usuarioLogado.getUsername());
            System.out.println("**************************************************************************");
            ...
            switch (escolha) {
                case 1: {
                    ...
                    System.out.println("-----------------------------------------------------------------------"); 
                    if (vendas.isEmpty()) {
                        System.out.println("VOCÊ NÃO POSSUI NENHUMA VENDA");
                        acessoAdmin(usuarios, clientes, empresas, produtos, carrinho, vendas, usuarioLogado);
                    }
                    vendas.stream().forEach(venda -> {
                        System.out.printf("               EMPRESA %s %n             ", venda.getEmpresa().getNome());
                ...
                case 2: {
                    ...
                    produtos.stream().forEach(produto -> {
                        System.out.println("-----------------------------------------------------------------------"); 
                        System.out.printf("EMPRESA: %s %n", produto.getEmpresa().getNome());
                        System.out.println("-----------------------------------------------------------------------\n"); 
        ...
        public static void acessoEmpresa(List<Usuario> usuarios, List<Cliente> clientes, List<Empresa> empresas,
			List<Produto> produtos, List<Produto> carrinho,List<Venda> vendas,Usuario usuarioLogado){
            System.out.println("\n**************************************************************************");
            System.out.printf("OLÁ %s, escolha uma opção para iniciar%n", usuarioLogado.getEmpresa().getNome());
            System.out.println("**************************************************************************");
            System.out.println("1 - Listar vendas");
            System.out.println("2 - Ver produtos");
            System.out.println("0 - Deslogar");
            ...
            switch (escolha) {
                case 1: {
                    ...
                    vendas.stream().forEach(venda -> {
                        if (venda.getEmpresa().getId().equals(usuarioLogado.getEmpresa().getId())) {
                            ...
                            System.out.printf("\nTotal Venda: R$ %.2f %n", venda.getValor());
                            System.out.printf("Total Taxa a ser paga: R$%.2f %n", venda.getComissaoSistema());
                            System.out.printf("Total Líquido  para empresa: R$ %.2f %n",
                                            (venda.getValor() - venda.getComissaoSistema()));
                        }else{
                            System.out.println("VOCÊ NÃO POSSUI VENDAS");
                            acessoEmpresa(usuarios, clientes, empresas, produtos, carrinho, vendas, usuarioLogado);
                    ...
                    System.out.println("------------------------------------------------------------------------");
                    System.out.printf("Saldo Empresa: %.2f %n", usuarioLogado.getEmpresa().getSaldo());
                    System.out.println("------------------------------------------------------------------------");
                ...
                case 2: {
                    ...
                    System.out.printf("Saldo Empresa: R$ %.2f %n", usuarioLogado.getEmpresa().getSaldo());
                    System.out.println("------------------------------------------------------------------------");
        ...
        public static void acessoCliente(List<Usuario> usuarios, List<Cliente> clientes, List<Empresa> empresas,
			List<Produto> produtos, List<Produto> carrinho,List<Venda> vendas,Usuario usuarioLogado){
            System.out.println("\n**************************************************************************");
            System.out.printf("OLÁ %s, escolha uma opção para iniciar%n", usuarioLogado.getCliente().getNome());
            System.out.println("**************************************************************************");
            ...
            switch (escolha) {
                case 1: {
                    ...
                    System.out.println(empresaEscolhida.getNome());
                ...
                case 2: {   
                    ...
                    System.out.println("------------------------------------------------------------------------");
                    if (vendas.isEmpty()) {
                        System.out.println("VOCÊ NÃO EFETUOU NENHUMA COMPRA");
                        acessoCliente(usuarios, clientes, empresas, produtos, carrinho, vendas, usuarioLogado);
                    }
                    vendas.stream().forEach(venda -> {
